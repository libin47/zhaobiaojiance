...
class DB_ZB(DBBase):
    ...
    def count_name(self, data):
        return self.db.query(ZhaoBiao).filter(ZhaoBiao.name==data).count()

    def count_name_time(self, name, time):
        # 检查同一标题的相同发布日期，可依此判断此条数据是否已重复（不同网站之间亦可）
        return self.db.query(ZhaoBiao).filter(ZhaoBiao.name==name, ZhaoBiao.date==time).count()

    def count_source_href(self, source, href):
        # 检查同一来源的相同链接，可依此判断此网页的爬取是否已重复
        return self.db.query(ZhaoBiao).filter(ZhaoBiao.source==source, ZhaoBiao.href==href).count()

    def find_no_send(self, city):
        return self.db.query(ZhaoBiao).filter(ZhaoBiao.send==False,ZhaoBiao.city==city).all()
