...
class DB_YX(DBBase):
    ...
    def count_name(self, data):
        return self.db.query(YiXiang).filter(YiXiang.name==data).count()

    def count_name_time(self, name, time):
        # 检查同一标题的相同发布日期，可依此判断此条数据是否已重复（不同网站之间亦可）
        return self.db.query(YiXiang).filter(YiXiang.name==name, YiXiang.date==time).count()

    def count_source_href(self, source, href):
        # 检查同一来源的相同链接，可依此判断此网页的爬取是否已重复
        return self.db.query(YiXiang).filter(YiXiang.source==source, YiXiang.href==href).count()

    def find_no_send(self, city):
        return self.db.query(YiXiang).filter(YiXiang.send==False, YiXiang.city==city).all()
    ...
    def batch_update_send_true(self, ids):
        if not ids:
            return
        self.db.query(YiXiang) \
            .filter(YiXiang.id.in_(ids)) \
            .update({YiXiang.send: True}, synchronize_session=False)
        self.db.commit()